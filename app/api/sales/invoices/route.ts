...
export async function GET() {
  ...
  return NextResponse.json({ invoices: await db.customerInvoice.findMany({ include: { lines: true, payments: true }, orderBy: { createdAt: "desc" } }) });
}
export async function POST(req: Request) {
  ...
  const inv = await db.$transaction(async (tx) => {
    const no = await nextInvNo(tx, parsed.data.invDate ? new Date(parsed.data.invDate) : new Date());
    return tx.customerInvoice.create({
      data: {
        ...
        invDate: parsed.data.invDate ? new Date(parsed.data.invDate) : new Date(),
